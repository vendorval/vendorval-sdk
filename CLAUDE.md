...
Both wrap the [VendorVal REST API](https://docs.vendorval.com) (`https://api.vendorval.com/v1`) and expose the same surface so the two languages stay at parity. VendorVal is a vendor-verification product by Modali Consulting; the API itself lives in a separate (private) `vendorval-api` repository, and `specs/openapi.json` is a snapshot mirrored from its releases.
